...
class ViewDeviceTicketsPage extends StatefulWidget {
  final String baseUrl;
  final String currentUserId;

  const ViewDeviceTicketsPage({
    Key? key,
  ...
  @override
  State<ViewDeviceTicketsPage> createState() => _ViewDeviceTicketsPageState();
}

class _ViewDeviceTicketsPageState extends State<ViewDeviceTicketsPage> {
  ...
  Future<void> _fetchTickets() async {
    ...
    try {
      final uri = Uri.parse(
        "${widget.baseUrl}/api/tickets/list?type=device&userId=${widget.currentUserId}",
      );
  ...
  // طباعة تذكرة واحدة بتصميم احترافي (مطابق لصفحة الموقع)
  Future<void> _printSingleTicket(Map<String, dynamic> ticket) async {
            ...
            children: [
              ...
              pw.Row(
                ...
                children: [
                  ...
                  // العمود الأيسر: بيانات الجهاز
                  pw.Expanded(
                    flex: 5,
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        _buildHeaderSection(font, 'بيانات الجهاز'),
                        pw.SizedBox(height: 10),
                        _buildProfessionalRow(
                          'نوع الجهاز',
                          _getFieldValue(ticket, ['deviceType', 'device_type']),
                          font,
                        ),
                        _buildProfessionalRow(
                          'السيريال نمبر',
                        ...
                        _buildProfessionalRow(
                          'اسم الجهاز',
                          _getFieldValue(ticket, ['deviceName', 'device_name']),
                          font,
                        ),
                        if (ticket['deviceLocation'] != null &&
                            ticket['deviceLocation'].toString().isNotEmpty)
                          _buildProfessionalRow(
                            'موقع الجهاز',
                            ticket['deviceLocation'].toString(),
                            font,
                          ),
                        _buildProfessionalRow(
                          'الوصف الفني',
    ...
    await Printing.sharePdf(
      ...
      filename: 'device_ticket_${ticket['id']}.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("تذاكر صيانة الأجهزة"),
        centerTitle: true,
        elevation: 0,
      ...
      body: _loading
          ...
          : _tickets.isEmpty
          ? const Center(child: Text("لا توجد تذاكر صيانة أجهزة"))
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final deviceType = _getFieldValue(ticket, [
                  'deviceType',
                  'device_type',
                ]);
                final serialNumber = _getFieldValue(ticket, [
                  'serialNumber',
                  'serial_number',
                ]);
                final deviceName = _getFieldValue(ticket, [
                  'deviceName',
                  'device_name',
                ]);
                        ...
                        children: [
                          ...
                          if (deviceType.isNotEmpty)
                            Padding(
                              ...
                              child: Text("نوع الجهاز: $deviceType"),
                            ),
                          if (serialNumber.isNotEmpty)
                            Text("السيريال: $serialNumber"),
